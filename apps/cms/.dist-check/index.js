...
var HonoRequest = /* @__PURE__ */ __name(class {
  ...
  async parseBody(options) {
    return parseBody2(this, options);
  }
  #cachedBody = (key) => {
...
var Hono = /* @__PURE__ */ __name(class _Hono {
  ...
  */
  ...
  route(path, app) {
    ...
    app.routes.map((r) => {
      let handler;
      if (app.errorHandler === errorHandler2) {
        handler = r.handler;
      } else {
...
var timestamps = {
  ...
  updatedAt: integer("updated_at", { mode: "timestamp_ms" }).notNull().default(sql`(unixepoch() * 1000)`).$onUpdate(() => /* @__PURE__ */ new Date())
};
...
var subscribers = sqliteTable(
  "subscribers",
  {
    id: text("id").primaryKey(),
    email: text("email").notNull().unique(),
    name: text("name"),
    subscribedAt: integer("subscribed_at", { mode: "timestamp_ms" }).notNull().default(sql`(unixepoch() * 1000)`),
    active: integer("active", { mode: "boolean" }).notNull().default(true)
  },
  (t) => ({
...
var auditLogs = sqliteTable("audit_logs", {
  ...
  // e.g. post.create, post.update, settings.save, login
...
var SESSION_COOKIE_NAME = "blog_session";
var MAX_AGE = 60 * 60 * 24 * 30;
var clearSessionCookie = `${SESSION_COOKIE_NAME}=; HttpOnly; Path=/; Max-Age=0; SameSite=Lax`;
...
async function findSession(db, cookieHeader) {
...
async function createSession(db, userId) {
  ...
  const tokenHash = await hashToken(token);
  const expiresAt = new Date(Date.now() + MAX_AGE * 1e3);
...
async function destroySession(db, cookieHeader) {
  const token = readSessionToken(cookieHeader);
  if (!token)
    return;
  const tokenHash = await hashToken(token);
...
var withDb = createMiddleware(async (c, next) => {
  c.set("db", createDb(c.env.DB));
  await next();
});
var loadUser = createMiddleware(async (c, next) => {
  const db = c.get("db");
...
__name(hashPasswordRaw, "hashPasswordRaw");
...
var postSchema = external_exports.object({
  ...
  excerpt: external_exports.string().max(500).optional().nullable(),
  content: external_exports.string().default(""),
  categoryId: external_exports.string().optional().nullable(),
  coverImage: external_exports.string().optional().nullable(),
  status: external_exports.enum(["draft", "published"]).default("draft"),
  pinned: external_exports.boolean().default(false),
  publishedAt: external_exports.string().datetime().optional().nullable(),
  // ISO
  tagIds: external_exports.array(external_exports.string()).default([]),
  seoTitle: external_exports.string().optional().nullable(),
  seoDescription: external_exports.string().optional().nullable()
});
...
var categorySchema = external_exports.object({
  ...
  parentId: external_exports.string().optional().nullable(),
  order: external_exports.number().optional().default(0)
});
var pageSchema = external_exports.object({
  title: external_exports.string().min(1).max(200),
  slug: external_exports.string().min(1).max(200).optional(),
  content: external_exports.string().default(""),
  status: external_exports.enum(["draft", "published"]).default("published"),
  menuOrder: external_exports.number().optional().default(0),
  showInMenu: external_exports.boolean().optional().default(false)
});
var linkSchema = external_exports.object({
  name: external_exports.string().min(1).max(100),
  url: external_exports.string().url(),
  description: external_exports.string().optional().nullable(),
  avatar: external_exports.string().optional().nullable(),
  order: external_exports.number().optional().default(0),
  status: external_exports.enum(["active", "hidden"]).default("active")
});
var subscriberCreateSchema = external_exports.object({
...
function slugify(input) {
  return input.toLowerCase().trim().replace(/[\s_]+/g, "-").replace(/[^a-z0-9\u4e00-\u9fff-]/g, "").replace(/-+/g, "-").replace(/^-|-$/g, "");
}
__name(slugify, "slugify");
function uniqueSlug(input, exists2) {
  return resolveUnique(slugify(input) || "untitled", exists2);
}
__name(uniqueSlug, "uniqueSlug");
async function resolveUnique(base, exists2) {
  let slug = base;
  let i = 1;
  while (await exists2(slug)) {
    slug = `${base}-${i}`;
    i++;
  }
  return slug;
}
__name(resolveUnique, "resolveUnique");
...
function authRoutes() {
  ...
  app.post("/login", async (c) => {
    ...
    const user = await db.select().from(users).where(or(eq(users.username, body.usernameOrEmail), eq(users.email, body.usernameOrEmail))).get();
...
async function listPosts(db, opts) {
  ...
  let where = conditions.length ? and(...conditions) : void 0;
  if (opts.tagId) {
    const tagged = db.select({ postId: postTags.postId }).from(postTags).where(eq(postTags.tagId, opts.tagId));
    where = sql`${posts.id} IN (${tagged})`;
    if (conditions.length)
      where = and(where, ...conditions);
  }
  const total = (await db.select({ count: sql`count(*)` }).from(posts).where(where).get())?.count ?? 0;
  const rows = await db.select().from(posts).where(where).orderBy(desc(posts.pinned), desc(posts.publishedAt), desc(posts.createdAt)).limit(limit).offset(offset).all();
...
async function createPost(db, data, authorId) {
  const id = createId();
  const slug = await uniqueSlug(data.slug || data.title, (s) => postSlugExists(db, s));
  const publishedAt = data.status === "published" ? new Date(data.publishedAt ?? Date.now()) : data.publishedAt ? new Date(data.publishedAt) : null;
...
async function updatePost(db, id, data, authorId) {
  ...
  const publishedAt = data.status === "published" ? new Date(data.publishedAt ?? existing.publishedAt ?? Date.now()) : data.publishedAt ? new Date(data.publishedAt) : existing.publishedAt;
...
async function updateTag(db, id, data) {
...
async function postSlugExists(db, slug, excludeId) {
  const row = await db.select({ id: posts.id }).from(posts).where(eq(posts.slug, slug)).get();
  return row ? row.id !== excludeId : false;
}
__name(postSlugExists, "postSlugExists");
async function tagSlugExists(db, slug, excludeId) {
  const row = await db.select({ id: tags.id }).from(tags).where(eq(tags.slug, slug)).get();
  return row ? row.id !== excludeId : false;
}
__name(tagSlugExists, "tagSlugExists");
async function categorySlugExists(db, slug, excludeId) {
  const row = await db.select({ id: categories.id }).from(categories).where(eq(categories.slug, slug)).get();
  return row ? row.id !== excludeId : false;
}
__name(categorySlugExists, "categorySlugExists");
async function getPostTags(db, postId) {
  return db.select({ id: tags.id, name: tags.name, slug: tags.slug }).from(postTags).innerJoin(tags, eq(postTags.tagId, tags.id)).where(eq(postTags.postId, postId)).all();
}
__name(getPostTags, "getPostTags");
...
__name(setPostTags, "setPostTags");
...
async function createPage(db, data) {
  const id = createId();
...
async function updatePage(db, id, data) {
...
async function listLinks(db, includeHidden = false) {
  const cond = includeHidden ? void 0 : eq(links.status, "active");
...
async function listSubscribers(db, q) {
  const likeQ = q ? `%${q}%` : void 0;
  const cond = likeQ ? like(subscribers.email, likeQ) : void 0;
  return db.select().from(subscribers).where(cond).orderBy(desc(subscribers.subscribedAt)).all();
}
__name(listSubscribers, "listSubscribers");
async function addSubscriber(db, data) {
  const existing = await db.select().from(subscribers).where(eq(subscribers.email, data.email)).get();
  if (existing)
...
function contentRoutes() {
  ...
  app.get(
    "/subscribers",
    async (c) => c.json({ success: true, data: await listSubscribers(c.get("db"), c.req.query("q")) })
  );
  app.post(
    "/subscribers",
    async (c) => c.json({ success: true, data: await addSubscriber(c.get("db"), await parseBody(c.req, subscriberCreateSchema)) })
  );
...
__name(contentRoutes, "contentRoutes");
...
var navItemSchema = external_exports.object({
  label: external_exports.string(),
  url: external_exports.string(),
  newWindow: external_exports.boolean().default(false)
});
var siteConfigSchema = external_exports.object({
  basic: external_exports.object({
    siteName: external_exports.string().default("My Blog"),
    tagline: external_exports.string().default(""),
    logo: external_exports.string().default(""),
    favicon: external_exports.string().default(""),
    language: external_exports.string().default("zh-CN"),
  ...
  seo: external_exports.object({
    ...
    ogImage: external_exports.string().default(""),
    enableSitemap: external_exports.boolean().default(true),
    enableRobots: external_exports.boolean().default(true)
  }),
  nav: external_exports.object({
    menu: external_exports.array(navItemSchema).default([])
  }),
  author: external_exports.object({
    name: external_exports.string().default(""),
    avatar: external_exports.string().default(""),
    bio: external_exports.string().default(""),
    socials: external_exports.array(socialSchema).default([])
  }),
  footer: external_exports.object({
    footerText: external_exports.string().default(""),
    beian: external_exports.string().default("")
  }),
  inject: external_exports.object({
    headHtml: external_exports.string().default(""),
    footHtml: external_exports.string().default("")
  }),
  features: external_exports.object({
    comments: external_exports.object({
      enabled: external_exports.boolean().default(false),
      provider: external_exports.enum(["self", "giscus", "utterances", "waline"]).default("self"),
      audit: external_exports.boolean().default(true),
      giscusRepo: external_exports.string().default(""),
      giscusRepoId: external_exports.string().default(""),
      giscusCategory: external_exports.string().default(""),
      giscusCategoryId: external_exports.string().default("")
    }),
    newsletter: external_exports.object({
      enabled: external_exports.boolean().default(false)
    }),
    analytics: external_exports.object({
      enabled: external_exports.boolean().default(false),
      webAnalyticsToken: external_exports.string().default("")
    })
  })
});
var DEFAULT_SITE_CONFIG = siteConfigSchema.parse({});
var SETTINGS_KEY = "site";
var CONFIG_CACHE_KEY = "config:site";
...
async function loadConfigCached(db, cache) {
  const cached = await cache.get(CONFIG_CACHE_KEY, "json");
  if (cached)
    return cached;
  const config2 = await loadConfig(db);
  await cache.put(CONFIG_CACHE_KEY, JSON.stringify(config2), { expirationTtl: 300 });
  return config2;
}
__name(loadConfigCached, "loadConfigCached");
async function saveConfig(db, cache, config2) {
  const valid = siteConfigSchema.parse(config2);
  await db.insert(settings).values({ key: SETTINGS_KEY, value: JSON.stringify(valid) }).onConflictDoUpdate({ target: settings.key, set: { value: JSON.stringify(valid) } });
...
function makeKey(filename, contentType) {
...
function mediaRoutes() {
  ...
  app.post("/upload", async (c) => {
    ...
    if (exceedsLimit(file.size))
      throw new ApiError(413, "File too large (max 20 MB)");
    if (!isAllowedType(file.type))
      throw new ApiError(415, `Unsupported type: ${file.type}`);
...
function renderInline(text2) {
  return text2.replace(/\*\*([^*]+)\*\*/g, "<strong>$1</strong>").replace(/(^|[^*])\*([^*]+)\*/g, "$1<em>$2</em>").replace(/`([^`]+)`/g, "<code>$1</code>").replace(/!\[([^\]]*)\]\(([^)\s]+)(?:\s+"([^"]*)")?\)/g, '<img src="$2" alt="$1" title="$3" />').replace(/\[([^\]]+)\]\(([^)\s]+)(?:\s+"([^"]*)")?\)/g, '<a href="$2" title="$3">$1</a>');
}
...
__name(renderFence, "renderFence");
function renderMarkdown(md) {
  ...
  while (i < lines.length) {
    ...
    const para = [line];
    i++;
    while (i < lines.length && lines[i].trim() !== "" && !/^(#{1,6})\s/.test(lines[i]) && !/^```/.test(lines[i]) && !/^\s*[-*+]\s+/.test(lines[i]) && !/^\s*\d+\.\s+/.test(lines[i]) && !/^\s*---+\s*$/.test(lines[i]) && !/^\s*>\s?/.test(lines[i])) {
      para.push(lines[i]);
      i++;
    }
...
function toPostView(p, tags2, withHtml) {
  return {
    id: p.id,
    title: p.title,
    slug: p.slug,
    ...
    pinned: p.pinned,
    tags: tags2.map((t) => t.slug),
    categoryId: p.categoryId,
    html: withHtml ? p.contentHtml ?? void 0 : void 0
  };
}
__name(toPostView, "toPostView");
function publicRoutes() {
  const app = new Hono2();
  app.use("*", withDb);
  app.get("/config", async (c) => {
    const config2 = await loadConfigCached(c.get("db"), c.env.CACHE);
    return c.json({ success: true, data: config2 });
  });
  app.get("/posts", async (c) => {
    ...
    const offset = (page - 1) * limit;
    const where = and(eq(posts.status, "published"), gte(posts.publishedAt, /* @__PURE__ */ new Date(1)));
    const total = (await db.select({ count: sql`count(*)` }).from(posts).where(where).get())?.count ?? 0;
    const rows = await db.select().from(posts).where(where).orderBy(desc(posts.pinned), desc(posts.publishedAt)).limit(limit).offset(offset).all();
    const items = await Promise.all(
      rows.map(async (p) => {
        const pt = await getPostTags(db, p.id);
        return toPostView(p, pt, false);
      })
    );
  ...
  app.get("/posts/:slug", async (c) => {
    ...
    const view = toPostView(row, pt, true);
    view.html = html;
  ...
  app.get("/links", async (c) => {
    const db = c.get("db");
    const rows = await db.select().from(links).where(eq(links.status, "active")).orderBy(links.order, links.name).all();
    return c.json({ success: true, data: rows });
  });
